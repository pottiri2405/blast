...
const ENEMIES = {
  ...
  'e': {'default': 'e'},
  'w': {'default': 'w'},
  'combine-parts-n': {'default': 'n'},
  'combine-parts-s': {'default': 's'},
  'combine-parts-e': {'default': 'e'},
  'combine-parts-w': {'default': 'w'}  
}
const MOVE_BOMBS = {
  'n': {'default': 'n'},
  's': {'default': 's'},
...
export default {
  ...
  methods: {
    ...
    moveObject (prefix, object) {
      let x = object.x
      let y = object.y
      if (object.type.startsWith('combine-parts-') && this.data.map[y][x] === 'enemy-combine-goal') {
        return
      }
      ...
      this.isRedBomb(x, y) ||
      (this.data.map[y][x] !== 'none' && this.data.map[y][x] !== 'enemy-combine-goal')
      ) {
    ...
    boxImages (x, y) {
      ...
      } else if (this.isEnemyCombined(x, y)) {
        images.push({src: base + 'enemy-combined.svg', class: 'combine', style: style})
      } else if (this.isEnemyCombineWait(x, y)) {
        images.push({src: base + 'enemy-combine-parts.svg', class: 'scale', style: style})        
      } else if (this.isEnemyCombineParts(x, y)) {
        images.push({src: base + 'enemy-combine-parts.svg', class: 'scale', style: style})
      } else if (this.isEnemy(x, y)) {
        if (this.data.map[y][x] !== 'enemy-combine-goal') {
    ...
    isEnemyCombineParts (x, y) {
      return (this.data.map[y][x].startsWith('enemy-combine-parts-'))
    },
    isEnemyCombineWait (x, y) {
      return (this.data.map[y][x] == 'enemy-combine-goal' && this.combine.total > 0 && this.combine.combined > 0 && this.combine.total > this.combine.combined)
    },        
    isEnemyCombined (x, y) {
      return (this.data.map[y][x] == 'enemy-combine-goal' && this.combine.total > 0 && this.combine.combined > 0 && this.combine.total <= this.combine.combined)
    },        
          ...
          for (let x in $vm.data.map[y]) {
            for (let [key, e] of Object.entries(ENEMIES)) {
              if ($vm.data.map[y][x] === 'enemy-' + key) {
                ...
                if ($vm.data.map[y][x].startsWith('enemy-combine-parts-')) {
                  $vm.combine.total++
                }                
              }
            }
